...
在提出解决 JavaScript 兼容性问题方案开始前，回顾 JavaScript 的发展史，以明确知道 JavaScript 兼容性问题的来源。
...
1. usebuiltins
   ...
   用于如何处理 polyfill，默认为 false，也就是禁用 polyfill。使用"usage"或者"entry"才会引入 core-js 作为 polyfill，而"usage"和"entry"的区别在于："usage"是按需引入（仅添加代码中实际用到的 API 的 polyfill），"entry"是全量引入（根据目标浏览器，一次性引入所有可能缺失的提案 polyfil）
...
- `babel-plugin-transform-runtime`
  ...
  `helpers`：是否启用辅助功能（如：`_extends`、`_asyncToGenerator`等）

  `regenerator`：是否启用生成器（generator）和 async 函数的支持
...
- `@babel/plugin-transform-block-scoped-functions`
  ...
  该插件用于将块级作用域的函数转换为普通函数

  <img src="./../.images/chapter-4/01_11.png" />

  配置项：

  `noShadow`：是否禁用变量提升，默认为 false
...
- `@babel/plugin-transform-destructuring`
  ...
  <img src="./../.images/chapter-4/01_12.png" />

  配置项：

  `useBuiltIns`：是否使用内置的解构赋值实现，默认为 false
...
- `@babel/plugin-transform-parameters`
  ...
  该插件用于转换参数语法，如默认参数、剩余参数等

  <img src="./../.images/chapter-4/01_13.png" />
...
- `@babel/plugin-transform-spread`
  ...
  <img src="./../.images/chapter-4/01_14.png" />
...
- `@babel/plugin-transform-template-literals`
  ...
  <img src="./../.images/chapter-4/01_15.png" />
...
- `@babel/plugin-transform-modules-commonjs`
  ...
  该插件用于将 ES6 模块语法转换为 CommonJS 模块语法

  <img src="./../.images/chapter-4/01_16.png" />
...
- `@babel/plugin-transform-modules-amd`
  ...
  该插件用于将 ES6 模块语法转换为 AMD 模块语法

  <img src="./../.images/chapter-4/01_17.png" />

  配置项：

  `amd`：是否启用 AMD 模块，默认为 true
...
- `@babel/plugin-transform-modules-systemjs`
  ...
  该插件用于将 ES6 模块语法转换为 SystemJS 模块语法

  <img src="./../.images/chapter-4/01_18.png" />

  配置项：

  `systemjs`：是否启用 SystemJS 模块，默认为 true
...
- `@babel/plugin-transform-runtime`
  ...
  <img src="./../.images/chapter-4/01_19.png" />
...
- **实现差异**：不同浏览器和 JavaScript 引擎对 ECMAScript 标准的实现进度和细节存在差异，尤其在标准演进初期和“浏览器大战”时期，导致大量兼容性问题。
- **标准与宿主环境分离**：ECMAScript 只规定语言本身，浏览器等宿主环境的 API（如 DOM/BOM）并不在标准范围内，各家实现差异更大。
- **新旧标准并存**：新语法和 API 推出后，旧环境不支持，开发者需兼顾多版本兼容。
- **可选特性与遗留特性**：标准允许部分特性为“实现定义”或“可选”，也保留部分遗留特性，进一步加剧兼容性挑战。

**如何查询兼容性：**

- 推荐使用 [Can I use](https://caniuse.com/) 查询主流浏览器对 ECMAScript 语法和 API 的支持情况，帮助开发者判断新特性在各环境中的可用性。
